...
int binary = 0, debug_mode = 2,min_left_size=1,max_left_size=3, window = 5, min_count = 5,num_threads = 12, min_reduce = 1;
int *vocab_hash;
...
void *TrainModelThread(void *id) {
	...
	while (1) {
		...
		if(b<min_left_size)b=min_left_size;
		if(b>max_left_size)b=max_left_size;
		//in -> hidden
		cw = 0;
		for (a=0; a<b; a++){
			c = sentence_position-b+1+a;
		...
		//learn prdict
		for (a=0; a<window-b; a++) {
			c = sentence_position+a+1;
			if (c < 0||c >= sentence_length)
				continue;
			last_word = sen[c];
			if (last_word == -1)
...
int main(int argc, char **argv) {
	int i;
	if (argc == 1) {
		printf("WORD VECTOR estimation toolkit v 0.1c,modify by xuen!\n\n");
		printf("Options:\n");
		...
		printf("\t-min-left-size <int>\n");
		printf("\t\tSet min left side word num to prdict next right side; default is 1\n");
		printf("\t-max-left-size <int>\n");
		printf("\t\tSet max left side word num to prdict next right side; default is 4\n");
		printf("\t-sample <float>\n");
		...
		printf("\nExamples:\n");
		printf("./word2vec -train data.txt -output vec.txt -size 40 -window 10 -max-left-size 6 -min-left-size 2 -sample 1e-4 -negative 10 -hs 0 -binary 0  -iter 3\n\n");
		return 0;
	}
	output_file[0] = 0;
	save_vocab_file[0] = 0;
	...
	if ((i = ArgPos((char *) "-min-left-size", argc, argv)) > 0){
		min_left_size= atoi(argv[i + 1]);
		if(min_left_size<1 || min_left_size>window){
			min_left_size=window-1;
		}
	}

	if ((i = ArgPos((char *) "-max-left-size", argc, argv)) > 0){
		max_left_size= atoi(argv[i + 1]);
		if(max_left_size<min_left_size || max_left_size>window){
			max_left_size=window-1;
		}
	}
